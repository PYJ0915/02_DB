...
WHERE (CLASS_NO, CLASS_TYPE) IN 
(SELECT CLASS_NO, CLASS_TYPE 
FROM TB_CLASS 
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
...
FROM(SELECT RANK() OVER(ORDER BY AVG(POINT) DESC) 순위, STUDENT_NO, STUDENT_NAME, AVG(POINT) 
FROM TB_STUDENT JOIN 
