...
-- (sys) VIEW 생성 권한을 부여 (sys -> kh_pyj)
GRANT CREATE VIEW TO kh_pyj;
...
-- DEPT_COPY2 테이블에서 DEPT_ID, LOCATION_ID 컬럼만 이용해 V_DCOPY VIEW 생성
...
--> 가상테이블 VIEW에 INSERT 가능

SELECT * FROM V_DCOPY2; -- INSERT 성공 확인
SELECT * FROM DEPT_COPY2; -- VIEW에 INSERT했으나 원본 테이블에도 INSERT됨!
-- => 모든 컬럼 값이 INSERT된 것이 아닌 VIEW를 생성할 때 사용한 컬럼에만 데이터가 삽입
-- 	  반대로, 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어감
-- 	  NULL은 DB의 무결성을 해치는 주요 원인이므로 가능하면 의도되지 않은 NULL은 존재하지 않게 해야함!

-- 무결성 : 데이터베이스에서 데이터를 정확하고 일관되게 유지하기 위한 중요한 개념 (데이터의 정확성, 일관성, 신뢰성 보장)

-- WITH READ ONLY 옵션 사용하기
-- VIEW를 이용해서 DML(INSERT/UPDATE/DELETE) 하는 것을 막기위해 사용
...
INCREMENT BY 5 -- 5씩 증가 => NEXTVAL 호출 시 5씩 증가
...
--> CURRVAL의 정확한 의미는 가장 최근 호출된 NEXTVAL 값을 반환하는 것
--> NEXTVAL을 한 번도 호출한 적이 없어 오류 발생!
--> 해결방법: NEXTVAL 호출

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 시퀀스 생성 후 첫 NEXTVAL은 START WITH에 지정한 값 (== 100)

-- NEXTVAL은 호출할 때마다 INCREMENT BY에 작성했던 수 만큼 증가 확인
...
WHERE TEST_NAME = '짱구';
-- 짱구의 TEST_NO 값을 150까지 증가시킨 후 NEXTVAL 호출
...
/*
 ...
 [CYCLE | NOCYCLE]
 [CACHE 시퀀스 개수 | NOCACHE]
 
*/

-- SEQ_TEST_NO의 MAXVALU 값을 200으로 수정
...
/* ROWID와 INDEX는 무슨 관계?
 * 인덱스가 ROWID를 저장함
 * 인덱스는 컬럼의 값과 해당 행의 ROWID를 매핑하여 저장하고 있음
 * 
 * 인덱스의 핵심 역할
 * 컬럼값을 해당행의 ROWID를 통해 빠르게 찾아내는 것
 * 
 * */
...
-- TB_IDX_TEST 테이블에 샘플 데이터 100만개 삽입(PL/SQL)
...
-- 보통 10~30배 정도 인덱스를 사용한 조회가 더 빠름!
