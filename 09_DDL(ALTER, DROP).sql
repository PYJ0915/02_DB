...
/* ALTER(바꾸다, 수정하다, 변경하다)
 * -- 테이블에서 수정할 수 있는 것
 * 1) 제약 조건(추가/삭제) * 제약 조건 자체를 수정하는 구문은 별도 존재하지 않음
 * 					  * 제약 조건 일부 수정은 불가 => 삭제 후 추가해야함! 
 * 2) 컬럼(추가/수정/삭제)
...
-- DEPARTMENT 테이블 복사 (컬럼명, 데이터 타입, NOT NULL 제약조건만 복사)
...
-- DEPT_COPY의 DEPT_TITLE 컬럼에 설정된 UNIQUE 제약조건 삭제
...
SELECT * FROM DEPT_COPY; --> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입 되었음을 확인

-- D10 개발 1팀 추가
INSERT INTO DEPT_COPY VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- ORA-12899: "KH_PYJ"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 데이터 타입이 CHAR(2) 이므로 영어+숫자 2글자 까지만 가능 BUT, D10은 3글자!
-- => VARCHAR2(3)으로 변경! (남는 바이트 메모리 반환을 위해서)

-- DEPT_ID 컬럼 데이터 타입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);

INSERT INTO DEPT_COPY VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- DEFAULT 값이 설정되어 있지 않아도 DEFAULT 사용 가능 (NULL)

SELECT * FROM DEPT_COPY; -- INSERT 성공 => 데이터 타입 변경 성공 확인

-- LNAME의 기본값을 'KOREA'로 수정
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';

SELECT * FROM DEPT_COPY; 
-- 기본값을 변경했다고 해서 기존 기본값으로 들어간 데이터가 변하진 않음
-- 새로 삽입될 행에서부터 새로운 기본값이 적용됨

-- LNAME '한국' -> 'KOREA'로 전부 변경
UPDATE DEPT_COPY 
SET LNAME = DEFAULT
WHERE LNAME = '한국';
...
-- DEPT_COPY 모든 컬럼 삭제
ALTER TABLE DEPT_COPY DROP(LNAME); -- LNAME 컬럼 삭제

SELECT * FROM DEPT_COPY; 

ALTER TABLE DEPT_COPY DROP COLUMN CNAME; -- CNAME 컬럼 삭제

SELECT * FROM DEPT_COPY; 

ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID; -- LOCATION_ID 삭제
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_TITLE; -- DEPT_TITLE 삭제
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_ID; -- DEPT_ID 삭제
...
SELECT * FROM DEPT_COPY; 
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 => 삭제 완료 확인
...
--> 해결방법
-- 1) 자식 테이블 삭제 후 부모 테이블 삭제
-- 2) ALTER를 통해 FK 제약조건 삭제 후 TB1 삭제
...
SELECT * FROM TB1;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 => 삭제 완료

SELECT * FROM TB2; -- 독립적 테이블로서 혼자 남게됨(아무 관계 없음...)

-------------------------------------------------------------------------------------

-- <DDL 주의사항>
-- 1) DDL은 COMMIT/ROLLBACK의 대상이 아님

-- 2) DDL과 DML구문을 섞어서 수행하면 안된다!
--> DDL 수행 시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴!
-- => DDL이 종료된 후에 DML구문을 수행할 수 있도록 하자(권장)
-- cf) DDL(CREATE/ALTER/DROP) : 객체 생성/수정/삭제
...
SELECT * FROM TB2; -- 롤백 불가! => DDL구문 중 ALTER를 사용한 시점에 자동으로 COMMIT되기 때문!
