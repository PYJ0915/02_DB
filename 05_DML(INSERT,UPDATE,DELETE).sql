-- 윈도우 > 설정 > 연결 > 연결유형 Auto commit by default 체크 해제 확인!
-- <주의> 혼자서 COMMIT, ROLLBACK 하지 말 것!
--------------------------------------------------------------------

-- ** DML(Data Manipulation Language) : 데이터 조작 언어 **
-- 테이블에 값을 삽입하거나(INSERT), 수정하거나(UPDATE), 삭제하는(DELETE) 구문

-- 테스트용 테이블 생성
CREATE TABLE EMPLOYEE2 AS SELECT * FROM EMPLOYEE;
CREATE TABLE DEPARTMENT2 AS SELECT * FROM DEPARTMENT;
-- => DEFAULT와 COMMENT값은 복사한 테이블에 따라오지 않음!

SELECT * FROM EMPLOYEE2;
SELECT * FROM DEPARTMENT2;

----------------------------------------------------------------------

-- 1. INSERT 
-- 테이블에 새로운 행을 추가하는 구문

-- 1) INSERT INTO 테이블명 VALUES(데이터, 데이터, 데이터...);
-- 테이블에 있는 모든 컬럼에 대한 값을 INSERT할 때 사용!

SELECT * FROM EMPLOYEE2;

INSERT INTO EMPLOYEE2 
VALUES('900', '홍길동', '991215-1234567', 'hong_gd@or.kr', '01011111111', 'D1', 'J7', 'S3', 4300000, 0.2, 200, SYSDATE, NULL, 'N');

ROLLBACK;

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900';

-- 2) INSERT IN 테이블명 (컬럼명1, 컬럼명 2, 컬럼명3 ...) VALUES (데이터1, 데이터2, 데이터3 ...);
-- 테이블에 내가 선택한 컬럼에 대한 값만 INSERT할 때 사용 
-- 선택되지 않은 컬럼은 NULL (DEFAULT 존재 시 DEFAULT 설정값으로 삽입됨)

INSERT INTO EMPLOYEE2 (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES ('900', '홍길동', '991215-1234567', 'hong_gd@or.kr', '01011111111', 'D1', 'J7', 'S3', 4300000);

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900';

COMMIT; -- 트랜젝션에 존재하던 홍길동 데이터 영구저장함
-- => 이미 DB에 영구저장 되어 ROLLBACK해도 되돌아가지 않음!

-- INSERT 시 VALUES 대신 서브쿼리 사용 가능
CREATE TABLE EMP_01(
	EMP_ID NUMBER, 
	EMP_NAME VARCHAR2(30), 
	DEPT_TITLE VARCHAR2(20) 	
); -- 테이블 생성 기본 문법

SELECT * FROM EMP_01;

SELECT EMP_ID, EMP_NAME, DEPT_TITLE 
FROM EMPLOYEE2
LEFT JOIN DEPARTMENT2 ON (DEPT_CODE = DEPT_ID);

-- 서브쿼리(SELECT) 결과(RESULT SET)를 EMP_01 테이블에 INSERT
--> SELECT 조회 결과의 데이터 타입, 컬럼 개수가 INSERT 하려는 테이블의 컬럼과 일치해야함! 

INSERT INTO EMP_01(
	SELECT EMP_ID, EMP_NAME, DEPT_TITLE 
	FROM EMPLOYEE2
	LEFT JOIN DEPARTMENT2 ON (DEPT_CODE = DEPT_ID)
);

SELECT * FROM EMP_01
ORDER BY EMP_ID;

---------------------------------------------------------------------------

-- 2. UPDATE (내용을 바꾸거나 추가해서 최신화)
-- 테이블에 기록된 컬럼의 값을 수정하는 구문

-- [작성법]
/*
 * UPDATE 테이블명
 * SET 컬럼명 = 바꿀값
 * [WHERE 컬럼명 비교연산자 비교값];
 * => WHERE 절은 옵션이지만 WHERE절 조건 중요! (WHERE절 미작성 시 전체 수정)
 * 
 * */

-- DEPARTEMENT2 테이블에서 DEPT_ID가 'D9'인 부서 정보 조회
SELECT * FROM DEPARTMENT2
WHERE DEPT_ID = 'D9';

-- DEPARTEMENT2 테이블에서 DPET_ID가 'D9'인 부서의 DEPT_TITLE을 '전략 기획팀으로 수정'
UPDATE DEPARTMENT2 
SET DEPT_TITLE = '전략기획팀'
WHERE DEPT_ID = 'D9';

SELECT * FROM DEPARTMENT2
WHERE DEPT_ID = 'D9';

-- * 조건절을 설정하지 않고 UPDATE구문 실행
UPDATE DEPARTMENT2 
SET DEPT_TITLE = '기술연구팀';

SELECT * FROM DEPARTMENT2;

-- EMPLOYEE2 테이블에서 BONUS를 받지않는 사원의 BONUS를 0.1로 수정

UPDATE EMPLOYEE2 
SET BONUS = '0.1'
WHERE BONUS IS NULL;

SELECT * FROM EMPLOYEE2;

ROLLBACK;

--------------------------------------------------------------------------

-- 여러 컬럼을 한 번에 수정할 시 ',' 로 컬럼을 구분하면 됨
-- D9 / 총무부 => D0 / 전략기획팀 수정
UPDATE DEPARTMENT2
SET DEPT_ID = 'D0', DEPT_TITLE = '전략 기획팀'
WHERE DEPT_ID = 'D9' AND DEPT_TITLE = '총무부';

SELECT * FROM DEPARTMENT2

--------------------------------------------------------------------------

-- * UPDATE 시에도 서브쿼리 사용 가능

-- [작성법]
/*
 * UPDATE 테이블명
 * SET 컬럼명 = (서브쿼리)
 * [WHERE 컬럼명 비교연산자 비교값];
 * 
 * */

-- EMPLOYEE2 테이블에서 방명수 사원의 급여와 보너스율을
-- 유재식 사원과 동일하게 변경해주기로했다. 이를 반영하는 UPDATE문을 작성해보자

-- 유재식 사원 급여 조회
SELECT SALARY FROM EMPLOYEE2
WHERE EMP_NAME = '유재식'; -- 3,400,000

-- 유재식 사원 보너스율 조회
SELECT BONUS FROM EMPLOYEE2
WHERE EMP_NAME = '유재식'; -- 0.2

-- 방명수 급여, 보너스 수정
UPDATE EMPLOYEE2 
SET SALARY = (
SELECT SALARY FROM EMPLOYEE2
WHERE EMP_NAME = '유재식') ,
BONUS = (
SELECT BONUS FROM EMPLOYEE2
WHERE EMP_NAME = '유재식')
WHERE EMP_NAME = '방명수';

SELECT EMP_NAME, SALARY, BONUS 
FROM EMPLOYEE2
WHERE EMP_NAME IN ('유재식', '방명수');

----------------------------------------------------------------------

-- 3. MERGE(병합)
-- 구조가 같은 두 개의 테이블을 하나로 합치는 기능
-- 테이블에서 지정하는 조건의 값이 존재하면 UPDATE 없으면 INSERT하는 구문

CREATE TABLE EMP_M01 
AS SELECT * FROM EMPLOYEE;

CREATE TABLE EMP_M02 
AS SELECT * FROM EMPLOYEE
WHERE JOB_CODE = 'J4';

SELECT * FROM EMP_M01; -- 23명
SELECT * FROM EMP_M02; -- 5명(기존 4명 + 신규 1명)

INSERT INTO EMP_M02
VALUES (999, '곽두원', '561016-1234567', 'kwack_dw@or.kr', '01011112222', 'D9', 'J4', 'S1', 9000000, 0.5, NULL, SYSDATE, NULL, NULL);

UPDATE EMP_M02 SET SALARY = '0';

MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID)
WHEN MATCHED THEN
UPDATE SET
	EMP_M01.EMP_NAME = EMP_M02.EMP_NAME,
	EMP_M01.EMP_NO = EMP_M02.EMP_NO,
	EMP_M01.EMAIL = EMP_M02.EMAIL,
	EMP_M01.PHONE = EMP_M02.PHONE,
	EMP_M01.DEPT_CODE = EMP_M02.DEPT_CODE,
	EMP_M01.JOB_CODE = EMP_M02.JOB_CODE,
	EMP_M01.SAL_LEVEL = EMP_M02.SAL_LEVEL,
	EMP_M01.SALARY = EMP_M02.SALARY,
	EMP_M01.BONUS = EMP_M02.BONUS,
	EMP_M01.MANAGER_ID = EMP_M02.MANAGER_ID,
	EMP_M01.HIRE_DATE = EMP_M02.HIRE_DATE,
	EMP_M01.ENT_DATE = EMP_M02.ENT_DATE,
	EMP_M01.ENT_YN = EMP_M02.ENT_YN
WHEN NOT MATCHED THEN
INSERT VALUES(EMP_M02.EMP_ID, EMP_M02.EMP_NAME, EMP_M02.EMP_NO, EMP_M02.EMAIL, 
	         EMP_M02.PHONE, EMP_M02.DEPT_CODE, EMP_M02.JOB_CODE, EMP_M02.SAL_LEVEL, EMP_M02.SALARY, EMP_M02.BONUS, EMP_M02.MANAGER_ID, EMP_M02.HIRE_DATE, 
	         EMP_M02.ENT_DATE, EMP_M02.ENT_YN);

SELECT * FROM EMP_M01; -- 없었던 것은 INSERT, 있었던 것은 UPDATE
-----------------------------------------------------------------------

-- 4. DELETE
-- 테이블의 행을 삭제하는 구문

-- [작성법]
-- DELETE FROM 테이블명 [WHERE 컬럼명 비교연산자 비교값]
-- 만약 WHERE 조건설정 하지 않을 시 전체 삭제!

COMMIT;

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = 900;

-- 홍길동 삭제해보기
DELETE FROM EMPLOYEE2
WHERE EMP_NAME = '홍길동';

ROLLBACK; -- 마지막 커밋 시점까지 되돌아감

-- EMPLOYEE2 테이블 행 전체 삭제
DELETE FROM EMPLOYEE2;

SELECT * FROM EMPLOYEE2

ROLLBACK;

SELECT * FROM EMPLOYEE2

------------------------------------------------------------------------------------------

-- 5. TRUNCATE(DML이 아닌 DDL)
-- 테이블 전체 행을 삭제하는 DDL (테이블 자체를 삭제하는 것 X)
-- DELETE 보다 수행속도가 더 빠름
-- DML이 아닌 DDL이므로 트랜잭션에 담기지 않아서 ROLLBACK을 통한 복구 불가!

-- 테스트용 테이블 생성
CREATE TABLE EMPLOYEE3
AS SELECT * FROM EMPLOYEE2;

-- 생성 확인
SELECT * FROM EMPLOYEE3;

-- TRUNCATE로 삭제
TRUNCATE TABLE EMPLOYEE3;

SELECT * FROM EMPLOYEE3; -- 행이 전부 삭제 됨을 확인!

ROLLBACK;

-- 롤백 후 복구 확인(TRUNCATE => 복구 불가!)
SELECT * FROM EMPLOYEE3; -- 여전히 전체 행 비어있음..

-- DELETE : 휴지통에 버리기 (복구 가능)
-- TRUNCATE : 완전 삭제 (복구 불가능)

