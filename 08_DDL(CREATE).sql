 ...
 *
 * DATE : 날짜 타입
 * BLOB : 대용량 이진 데이터 (4GB) => BINARY
 * CLOB : 대용량 문자 데이터 (4GB) => CHARACTER
 *
 * */
...
INSERT INTO MEMBER VALUES('MEM02', 'QWER1234', '영희킴', '000123-2235567', SYSDATE);

--> 가입일 INSERT 시 미작성하는 경우 DEFAULT값 반영 되는지 획인
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) VALUES('MEM03', 'ZXCV1234', '이지연');
...
--> NUMBER 타입 컬럼에 데이터 삽입 시 제일 앞에 0이 있으면 이를 자동으로 제거
--> 전화번호, 주민등록번호처럼 숫자로만 되어있는 데이터라도 0으로 시작할 가능성이 있다면 문자 타입(CHAR, VARCHAR2)을 이용해야 함!
...
CREATE TABLE USER_USED_NN(
	USER_NO NUMBER NOT NULL, -- 사용자 번호(모든 사용자는 사용자번호가 있어야함!)
	USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(20),
	USER_NAME VARCHAR2(30),
	USER_GENDER VARCHAR(10),
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(50) -- 마지막 컬럼 작성 부분까지 컬럼레벨
	-- 테이블 레벨
);
...
-- ORA-01400: NULL을 ("KH_PYJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
 ...
 -- USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- 컬럼 레벨(제약조건명 지정)
	...
	USER_GENDER VARCHAR(10),
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(50), -- 테이블 레벨에 작성하려면 마지막 컬럼 레벨에 ',' 작성해야함!
	-- 테이블 레벨
...
INSERT INTO USER_USED_UK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_PYJ.USER_ID_U)에 위배됩니다

INSERT INTO USER_USED_UK VALUES
(1, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
--> 아이디에 NULL 값 삽입 가능 확인(UNIQUE 제약 조건이 있어도 NULL값 허용)

INSERT INTO USER_USED_UK VALUES
(1, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
--> 아이디에 NULL 값 중복 삽입 가능 확인(UNIQUE 제약조건 NULL값은 중복 허용)
...
INSERT INTO USER_USED_UK2 VALUES
(1, 'USER02', 'PASS01', '고길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_PYJ.USER_ID_NAME_U)에 위배됩니다
--> 복합키로 지정된 컬럼 값 중 하나라도 다르면 위배되지 않음! (모든 컬럼값이 중복일 시 위배된다!)
...
-- NOT NULL + UNIQUE -> 중복되지 않는 값이 필수로 존재해야함!

-- 한 테이블 당 한 개만 설정 가능 (하나의 컬럼에만 지정 가능한 것 X)
-- 테이블 레벨, 컬럼 레벨 두 방식 다 가능!
-- 한 개 컬럼에 설정할 수 있고, 복합키로도 설정이 가능하다! (PRIMARY KEY 복합키)

CREATE TABLE USER_USED_PK(
 -- USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨(제약조건명 지정, 당연히 미지정도 가능)
	USER_NO NUMBER,
	USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(20),
	USER_NAME VARCHAR2(30),
...
INSERT INTO USER_USED_PK VALUES
(1, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_PYJ.USER_NO_PK)에 위배됩니다
--> 기본키의 중복으로 인한 오류!

INSERT INTO USER_USED_PK VALUES
(NULL, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- NULL을 ("KH_PYJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
--> 기본키에 NULL이 삽입되어 발생하는 오류!

INSERT INTO USER_USED_PK VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- 삽입 가능! (무결한 데이터)
...
INSERT INTO USER_USED_PK2 VALUES
(1, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr'); 
-- 복합키이기 때문에 USER_NO, USER_ID 둘 중 하나만 달라도 가능!
...
INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_PYJ.PK_USERNO_USERID)에 위배됩니다
--> USER_NO, USER_ID 둘 다 중복되었을 때만 제약조건 위배!

INSERT INTO USER_USED_PK2 VALUES
(NULL, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- ORA-01400: NULL을 ("KH_PYJ"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

INSERT INTO USER_USED_PK2 VALUES
(2, NULL, 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr');
-- ORA-01400: NULL을 ("KH_PYJ"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다

-- => PRIMARY KEY 복합키의 경우 둘 중 하나만 NULL이어도 위배!
...
-- 컬럼 레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 제약조건명] REFERENCES 참조할 테이블명 [(참조할 컬럼명)] [삭제룰]

-- 테이블 레벨일 경우
-- [CONSTRAINT 제약조건명] FOREIGN KEY (적용할 컬럼명) REFERENCES 참조할 테이블명 [(참조할 컬럼명)] [삭제룰]

-- * 참조될 수 있는 컬럼은 PRIMARY KEY 컬럼과, UNIQUE로 지정된 컬럼만 외래키로 사용 가능! *
-- 참조할 테이블에 참조할 컬럼명이 생략되면, 참조할 테이블의 PRIMARY KEY로 설정된 컬럼을 자동 참조함!
...
CREATE TABLE USER_USED_FK(
	USER_NO NUMBER PRIMARY KEY, -- 사용자번호(고유번호 => 중복 X / NULL X)
	...
	-- 참조할 컬럼명 미작성 시 USER_GRADE 테이블의 PK 참조
	-- 테이블 레벨
	-- , CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE /*(GRADE_CODE)*/
	--> FOREIGN KEY (적용할 컬럼명) 명령어는 테이블 레벨에서만 사용!
);

INSERT INTO USER_USED_FK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr', 10);
-- USER_GRADE(부모테이블/참조테이블)에 10이라는 GRADE_CODE가 존재하므로, 자식테이블이 10이라는 값 사용 가능! 

INSERT INTO USER_USED_FK VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-1592-1597', 'lee@kr.or.kr', 10);
-- USER_GRADE(부모테이블/참조테이블)에 10이라는 GRADE_CODE가 존재하므로, 자식테이블이 10이라는 값 사용 가능! 

INSERT INTO USER_USED_FK VALUES
(3, 'USER03', 'PASS03', '유관순', '여자', '010-3333-1111', 'yoo@kr.or.kr', 30);
-- USER_GRADE(부모테이블/참조테이블)에 30이라는 GRADE_CODE가 존재하므로, 자식테이블이 30이라는 값 사용 가능!

INSERT INTO USER_USED_FK VALUES
(4, 'USER04', 'PASS04', '안중근', '남자', '010-2222-1111', 'ann@kr.or.kr', NULL);
-- NULL 사용 가능!

INSERT INTO USER_USED_FK VALUES
(5, 'USER05', 'PASS05', '윤봉길', '남자', '010-7777-1111', 'yoon@kr.or.kr', 50);
-- ORA-02291: 무결성 제약조건(KH_PYJ.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
-- => 부모의 GRADE_CODE에 50이라는 값이 존재하지 않음!

-----------------------------------------------------------------------------------------

-- * FOREIGN KEY 삭제 옵션
-- 부모 테이블의 데이터 삭제 시 자식 테이블의 데이터를 어떤 식으로 처리할 것인지 내용 설정

-- 1) ON DELETE RESTRICTED(삭제 제한)로 기본 지정되어 있음.
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우 부모가 제공하고 있는 컬럼의 값은 삭제 불가!

DELETE FROM USER_GRADE WHERE GRADE_CODE = 30;
-- ORA-02292: 무결성 제약조건(KH_PYJ.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

DELETE FROM USER_GRADE WHERE GRADE_CODE = 20;
-- GRADE_CODE 중 20은 외래키로 참조되고 있지 않기 때문에 삭제 가능

-- 2) ON DELETE SET NULL : 부모키 삭제 시 자식키의 값을 NULL로 변경!
...
SELECT * FROM USER_USED_FK2; -- 10을 사용하고 있던 자식값이 NULL => ON DELETE SET NULL가 삭제룰이기 때문
...
SELECT * FROM USER_USED_FK3; 
-- 부모의 10을 참조하고 있던 자식 행 삭제됨을 확인!

-- cf) DML 이후 DDL 실행 시 자동으로 영구 저장됨! -> 그래서 보통 DML 전부 수행 후 DDL 수행
------------------------------------------------------------------------------------

-- 5. CHECK 제약조건 : 컬럼에 기록되는 값에 조건 설정을 할 수 있음
-- CHECK(컬럼명 비교연산자 비교값)
-- EX) GENDER -> CHECK(USER_GENDER IN('남', '여'))
...
CREATE TABLE USER_USED_CHECK(
	...
	USER_NAME VARCHAR2(30),
	USER_GENDER VARCHAR(10) CONSTRAINT GENDER_CHECK CHECK(USER_GENDER IN('남', '여')),
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(50)
	-- 테이블 레벨
	--, CONSTRAINT GENDER_CHECK CHECK(USER_GENDER IN('남', '여'))
);

INSERT INTO USER_USED_CHECK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_PYJ.GENDER_CHECK)이 위배되었습니다 => 체크 제약조건에 맞게 작성해야함

INSERT INTO USER_USED_CHECK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-5678', 'hong@kr.or.kr'); -- 가능

INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', '여자', '010-3333-1111', 'lee@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_PYJ.GENDER_CHECK)이 위배되었습니다 => 체크 제약조건에 맞게 작성해야함

INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', '여', '010-3333-1111', 'lee@kr.or.kr'); -- 가능

--> GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 삽입 가능하도록 설정함
--> 설정해둔 값 외의 값이 들어오면 제약조건이 위배되어 오류 발생!
...
/* [조건]
 1. EMP_COPY 테이블은 EMPLOYEE 테이블 구조만 복사하여 생성한다.
 2. EMP_COPY의 EMAIL 컬럼에 UNIQUE 제약조건을 추가한다.
 3. 추가한 제약조건을 이름 EMP_EMAIL_UQ 으로 지정한다.
 4. 이후 해당 제약조건을 삭제한다.
*/
CREATE TABLE EMP_COPY AS SELECT * FROM EMPLOYEE;
ALTER TABLE EMP_COPY ADD CONSTRAINT EMP_EMAIL_UQ UNIQUE(EMAIL);
ALTER TABLE EMP_COPY DROP CONSTRAINT EMP_EMAIL_UQ;
