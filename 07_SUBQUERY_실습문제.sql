...
(SELECT JOB_CODE, DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '노옹철')
AND EMP_NAME <> '노옹철';
...
(SELECT DEPT_CODE, MANAGER_ID FROM EMPLOYEE WHERE EMP_NO LIKE '77%' AND SUBSTR(EMP_NO, 8, 1) = '2');

-- 6. 부서별 입사일이 가장 빠른 사원의 사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일을 조회하고 
...
-- 7. 직급별 나이가 가장 어린 직원의 사번, 이름, 직급명, 나이, 보너스 포함 연봉을 조회하고 
-- 나이순으로 내림차순 정렬하세요 단, 연봉은 \124,800,000 으로 출력되게 하세요. (\ : 원 단위 기호)
