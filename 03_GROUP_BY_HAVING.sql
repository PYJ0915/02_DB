...
-- EMPLOYEE 테이블에서 (FROM)
-- 직급코드가 같은 사람의 (GROUB BY)
-- 직급코드, 급여 평균, 인원수를 직급코드 (SELECT)
-- 오름차순으로 조회 (ORDER BY)
...
-- EMPLOYEE 테이블에서
-- 성별(남/여)과 성별 별 인원수, 급여 합을
-- 인원 수 오름차순으로 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, COUNT(*) "인원 수", SUM(SALARY) "급여 합계"
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X (SELECT 해석 되기 전)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT절 해석 완료 후)

-----------------------------------------------------------------------------------------------------------
-- * WHERE절 GROUP BY 절 혼합하여 사용하기 *

--> WHERE절은 각 컬럽값에 대한 조건
--> HAVING절은 그룹에 대한 조건

-- EMPLOYEE 테이블에서 (FROM절)
-- 부서 코드가 D5, D6인 부서의 (WHERE절 -> GROUP BY절)
-- 부서 코드, 평균 급여, 인원 수 조회 (SELECT)

/* 4 */ SELECT DEPT_CODE, ROUND(AVG(SALARY)), COUNT(*) -- 부서 코드, 평균 급여, 인원수를 조회 
/* 1 */ FROM EMPLOYEE -- EMPLOYEE 테이블에서
/* 2 */ WHERE DEPT_CODE IN('D5', 'D6') -- 부서코드가 'D5', 'D6'인
/* 3 */ GROUP BY DEPT_CODE; -- 부서의(WHERE절에서 구한 조건을 그룹으로 묶음)

-- EMPLOYEE 테이블에서 (FROM)
-- 2000년도 이후 입사자들의 (WHERE)
-- 직급 코드, 직급별 급여 합을 조회해라 (GROUP BY AND SELECT)
...
-- EMPLOYEE 테이블에서
-- 부서별로 같은 직급은 사원의 인원수를 조회
-- 부서코드 오름차순, 직급코드 내림차순으로 정렬
-- 부서코드, 직급코드, 인원수

SELECT DEPT_CODE, JOB_CODE, COUNT(*) 
FROM EMPLOYEE 
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹화(대분류), 나눠진 그룹 내 JOB_CODE로 그룹화(소분류)
ORDER BY DEPT_CODE, JOB_CODE DESC;

-- GROUP BY 사용 시 주의 사항 SELECT문에 GROUP BY절을 사용할 경우
-- 그룹함수가 적용되지 않은 컬럼을 SELECT절에 명시한 조회하려는 컬럼 중 모두 GROUP BY절에 작성해야함!
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 => 한 사람의 급여가 300만원 이상이라는 조건(요구 사항 X)
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 평균이 300만원 이상인 그룹만 조회(요구사항 O)
ORDER BY DEPT_CODE;

-- EMPLOYEE 테이블에서
-- 직급별 인원 수가 5명 이하인 직급의
-- 직급 코드와 인원 수 조회
-- 단, 직급코드 오름차순 정렬
...
-- UNION ALL
-- MINUS
-- EMPLOYEE 테이블에서 급여가 300만 초과인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE 
...
FROM EMPLOYEE
WHERE SALARY > 3000000;
-->  ORA-01789: 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다. => 조회하는 컬럼의 개수가 맞지 않음!
--> ORA-01790: 대응하는 식과 같은 데이터 유형이어야 합니다 => 조회하는 컬럼의 타입이 맞지 않음!

-- 서로 다른 테이블이지만 컬럼의 타입과 개수만 일치하면 집합 연산자 사용 가능!
